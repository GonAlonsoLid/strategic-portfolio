...
def run_join_prediction(
    ...
    *,
    ...
    """Train join models over rolling windows; return (scores panel, metrics table)."""
    ...
    if not feat_cols:
        feat_cols = [c for c in features_join.select_dtypes(include=[np.number]).columns if c != "label_join"]
    X = features_join[feat_cols].fillna(0)
    y = features_join["label_join"]
    date_idx = features_join["date"]
    permno = features_join["permno"]
    ...
    out_scores = output_scores_path or processed / "join_scores.parquet"
    out_metrics = output_metrics_path or tables / "model_performance_join.csv"
    try:
